...
function getUrlParameter(name) {
  ...
  if (name === "music")
    return "https://cdn.shopify.com/s/files/1/0757/9700/4572/files/MaiMaiBenNhau-NooPhuocThinh-3602531.mp3?v=1748453356"; // URL mặc định cho nhạc
...
// Ảnh mặc định (đặt chung thư mục với index.html)
const defaultHeartImages = [
  ...
  "hinh33.jpg",
  "hinh34.jpg",
  "hinh35.jpg",// ảnh của bạn trong cùng thư mục
  // Bạn có thể thêm nhiều ảnh nếu muốn:
  // "hinh2.jpg",
  // "images/hinh3.jpg",
];

// Lấy ảnh từ URL parameter hoặc sử dụng ảnh mặc định
const urlImage = getUrlParameter("image");
let urlImages = [];

// Nếu có ảnh từ URL → dùng ảnh đó
if (urlImage) {
  urlImages = urlImage
    .split(",")
    .map((img) => img.trim())
  ...
  // Ngược lại → dùng ảnh trong thư mục (mặc định)
  urlImages = defaultHeartImages;
}


const heartImages = [
  ...(window.dataCCD?.data?.heartImages || []),
  ...(urlImages.length > 0 ? urlImages : defaultHeartImages),
];
...
// Lấy text từ URL parameter
const urlRingTexts = getUrlParameter("name");
let ringTexts = [];

if (urlRingTexts) {
  // Nếu có text từ URL, split theo dấu phẩy và trim khoảng trắng
  ringTexts = urlRingTexts
    .split(",")
    .map((text) => text.trim())
    .filter((text) => text.length > 0);
} else {
  // Nếu không có từ URL, sử dụng text mặc định
  ringTexts = [
    "Việt Nam",
    "Tomdev88",
    "Yêu các bạn",
...
function preloadGalaxyAudio() {
  // Lấy URL nhạc từ parameter hoặc sử dụng mặc định
  const urlMusic = getUrlParameter("music");
  const audioSources = urlMusic ? [urlMusic] : ["AE THEM NHAC TUY NHA"];
...
function createHintText() {
  ...
  const context = canvas.getContext("2d");
  const fontSize = 50;
  // Lấy text từ URL parameter hoặc sử dụng text mặc định
  const text = getUrlParameter("introduce") || "Chạm Vào Tinh Cầu";
